...
// A device without a GPU: every texture is an empty stub, presentation is always skipped and
// readbacks return zeros. Backs GSRendererNull and the software renderer when nothing needs to
// be displayed (headless trace runs, the browser build until a WebGPU device exists).
...
public:
	...
	void DrawMultiStretchRects(const MultiStretchRect* rects, u32 num_rects, GSTexture* dTex, ShaderConvertSelector shader = ShaderConvert::COPY) override;
...
protected:
	...
	void DoStretchRect(GSTexture* sTex, const GSVector4& sRect, GSTexture* dTex, const GSVector4& dRect, ShaderConvertSelector shader, Filter filter) override;
	void DoStretchRect(GSTexture* sTex, const GSVector4& sRect, const GSVector4& dRect, PresentShader shader, Filter filter) override;
};
